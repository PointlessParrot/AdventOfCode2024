...
namespace Day13B
{
    internal class Day13B
    {
        ...
        static long[,] Gridify(string[] strings)
        {
            ...
            {
                for (int j = 0; j < 3; j++)
                    grid[i, j] = long.Parse(input[j][i]);

                grid[i, 2] += 10000000000000;
            }

            return grid;
        }

        static void SolveGrid(long[,] grid)
        {
            long a1 = grid[0, 0];
            long a2 = grid[1, 0];
        ...
        static long GetAnswers(long[,] grid)
        {
            long b = grid[0, 1];
            if (grid[0, 2] % b != 0)
                return 0;

            long valueB = grid[0, 2] / b;
        ...
        static void Main(string[] args)
        {
            ...
            {
                long[,] grid = Gridify(lines.Skip(k).Take(3).ToArray());
                SolveGrid(grid);
